...
int main(int argc, char* argv[]) {
    ...
    if (argc < 7) {
        std::cerr << "Usage: " << argv[0] << " <qubits> <tau> <T0> <T1> <T2> <shots> <qasm filepath>\n";
        return 1;
    }
    ...
    bool qasm_exists = (argv[7] != nullptr) && (argv[7][0] != '\0');
    ...
    if(qasm_exists)
    {
        ...
        for(int n = 0; n < n_qubits; n++)
            circuit->COMB(n, p_exc, p_phase, p_amp);
    }
    ...
    else
    {
        ...
        {
            circuit->COMB(q, p_exc, p_phase, p_amp);
        }
    }
