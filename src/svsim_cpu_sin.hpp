...
public:
    Simulation(IdxType _n_qubits=0, IdxType _n_cpus=1) 
        ...
        results(NULL)
    {
        if (_n_qubits == 0) update(N_QUBIT_SLOT, 0);
...
inline void C1_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, 
...
void Purity_Check(const Gate& g, const IdxType t, const Simulation* sim, ValType* sv_real, ValType* sv_imag)
{
    ValType purity = 0; 
    for (IdxType i=0; i<(((IdxType)1<<(sim->n_qubits))); i++)
        purity += ((sv_real[i-1]*sv_real[i-1]) + (sv_imag[i-1]*sv_imag[i-1]));
    if ( abs(purity - 1.0) > ERROR_BAR )
