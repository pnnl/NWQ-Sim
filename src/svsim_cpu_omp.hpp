...
// OpenMP CPU based state-vector simulation 
// ---------------------------------------------------------------------------
#ifndef SVSIM_CPU_OMP_HPP_
#define SVSIM_CPU_OMP_HPP_
#include <assert.h>
#include <vector>
#include <math.h>
#include <cstdlib>
...
inline void M_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, const ValType rand, const IdxType qubit)
{
    ...
    for (IdxType i=0; i<((IdxType)1<<(sim->n_qubits)); i++)
    {
        ...
        else m_real[i] = (sv_real[i]*sv_real[i]) + (sv_imag[i]*sv_imag[i]); //square
    }
    #pragma omp barrier
