...
namespace NWQSim
{
    ...
    {
    public:
        Circuit(IdxType _n_qubits = 0) : n_qubits(_n_qubits), n_gates(0), circuit_gpu(NULL)
        {
        }
        ~Circuit() { clear(); }
        void append(Gate &g)
...
#pragma unroll
                                           ...
                                           const IdxType qubit2, const IdxType qubit3)
    {
        ...
        for (IdxType i = (sim->i_gpu) * per_pe_work + (warpid << 3); i < (sim->i_gpu + 1) * per_pe_work; i += (nwarps << 3))
        {
            ...
            for (int j = 0; j < 8; j++)
            {
                const IdxType term = get_term(i + j, p, q, r, s);
                if (laneid < 16)
                    el_real_s[j * 16 + laneid] = PGAS_G(sv_real, term + SV16IDX(laneid));
                else
                    el_imag_s[j * 16 + laneid - 16] = PGAS_G(sv_imag, term + SV16IDX(laneid - 16));
            }
            __syncwarp();
                ...
                if ((per_pe_num * (sim->i_gpu)) <= base && base < (per_pe_num * (sim->i_gpu + 1)))
                {
                    for (int j = 0; j < 8; j++)
                    {
                        ...
                        if (laneid < 16)
                        {
                            ...
                            {
                                if ((laneid & 1) != 0)
                                    el_real_s[j * 16 + laneid] = LOCAL_G(sv_real_remote, addr);
                                else
                        ...
                        else
                        {
                            ...
                            {
                                if ((laneid & 1) != 0)
                                    el_imag_s[j * 16 + laneid] = LOCAL_G(sv_imag_remote, addr);
                                else
                                    el_imag_s[j * 16 + laneid] = LOCAL_G(sv_imag, addr);
                            }
                            else // qubit2 is remote (not possible qubit0 or 1 is remote)
                            {
                                if (((laneid >> 1) & 1) != 0)
                                      ...
                                      const IdxType qubit, const ValType rand)
    {
        ...
        BARR;
        if (tid == 0)
            sim->results_gpu[0] = (rand <= prob_of_one ? 1 : 0);
    }
    //*/
