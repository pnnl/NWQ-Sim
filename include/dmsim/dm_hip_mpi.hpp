...
namespace NWQSim
{
    ...
    class DM_HIP_MPI;
    __global__ void dm_simulation_kernel_hip_mpi(DM_HIP_MPI *dm_gpu, IdxType n_gates, IdxType n_qubits, bool enable_tc);

    class DM_HIP_MPI : public QuantumState
    {
    public:
        DM_HIP_MPI(IdxType _n_qubits) : QuantumState(_n_qubits)
        {
            // Initialize the GPU
            n_qubits = _n_qubits;
            ...
            // CPU side initialization
            SAFE_ALOC_HOST_HIP(dm_real_cpu, dm_size_per_gpu);
            SAFE_ALOC_HOST_HIP(dm_imag_cpu, dm_size_per_gpu);
            ...
            dm_real = (ValType *)roc_shmem_malloc(dm_size_per_gpu);
            dm_imag = (ValType *)roc_shmem_malloc(dm_size_per_gpu);
            m_real = (ValType *)roc_shmem_malloc(dm_size_per_gpu);
            m_imag = (ValType *)roc_shmem_malloc(dm_size_per_gpu);
            hipCheckError();
            gpu_mem += dm_size_per_gpu * 4;

            // GPU memory initilization
            hipSafeCall(hipMemcpy(dm_real, dm_real_cpu, dm_size_per_gpu,
                                    hipMemcpyHostToDevice));
            hipSafeCall(hipMemcpy(dm_imag, dm_imag_cpu, dm_size_per_gpu,
                                    hipMemcpyHostToDevice));
            hipSafeCall(hipMemset(m_real, 0, dm_size_per_gpu));
            hipSafeCall(hipMemset(m_imag, 0, dm_size_per_gpu));
            rng.seed(time(0));
        }
        ...
        ~DM_HIP_MPI()
        {
            ...
            roc_shmem_free(dm_real);
            roc_shmem_free(dm_imag);
        ...
        void sim(std::shared_ptr<NWQSim::Circuit> circuit) override
        {
            ...
            unsigned smem_size;
            if (is_tc)
                smem_size = THREADS_CTA_HIP / 32 * 8 * 16 * 2 * sizeof(ValType); //TODO: Check the warp-size
            else
                smem_size = 0;

            int numBlocksPerSm = 1;
            // hipSafeCall(hipOccupancyMaxActiveBlocksPerMultiprocessor(&numBlocksPerSm,
            //                                                            dm_simulation_kernel_hip_mpi, THREADS_CTA_HIP, smem_size));
            ...
            hipSafeCall(hipLaunchCooperativeKernel((const void *)dm_simulation_kernel_hip_mpi, gridDim, THREADS_CTA_HIP, args, smem_size, 0));
        ...
        {
            throw std::logic_error("get_exp_z Not implemented (DM_HIP_MPI)");
        }
        ...
        void print_res_state() override
        {
            ...
            hipCheckError();
            hipSafeCall(hipMemcpy(dm_real_cpu, dm_real, dm_size_per_gpu, hipMemcpyDeviceToHost));
            hipSafeCall(hipMemcpy(dm_imag_cpu, dm_imag, dm_size_per_gpu, hipMemcpyDeviceToHost));

            ValType *dm_diag_real = NULL;
            ValType *dm_diag_imag = NULL;
            if (i_proc == 0)
                SAFE_ALOC_HOST_HIP(dm_diag_real, dim * sizeof(ValType));
            if (i_proc == 0)
                SAFE_ALOC_HOST_HIP(dm_diag_imag, dim * sizeof(ValType));
            ...
            if (i_proc == 0)
            {
                ...
                printf("\n");
                SAFE_FREE_HOST_HIP(dm_diag_real);
                SAFE_FREE_HOST_HIP(dm_diag_imag);
            }
        }
    ...
    public:
        ...
        __device__ __inline__ void C2V1_GATE(const ValType *gm_real, const ValType *gm_imag, const IdxType qubit0, const IdxType qubit1)
        {
            ...
            else
            {
                ...
                if (tid == 0)
                    roc_shmem_ctx_double_get(*p_ctx, dm_real_remote, dm_real, per_pe_num, pair_gpu); //TODO: remove dm_real_remote
                if (tid == 0)
                    roc_shmem_ctx_double_get(*p_ctx, dm_imag_remote, dm_imag, per_pe_num, pair_gpu); //TODO: remove dm_real_remote
                grid.sync();
                     ...
                     i += blockDim.x * gridDim.x)
                {
                    ...
                    el_real[0] = LOCAL_G_HIP_MPI(dm_real, term + SV4IDX(0));
                    el_imag[0] = LOCAL_G_HIP_MPI(dm_imag, term + SV4IDX(0));
                    el_real[3] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV4IDX(3));
                    el_imag[3] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV4IDX(3));

                    if (qubit0 == q) // qubit0 is the remote qubit
                    {
                        el_real[1] = LOCAL_G_HIP_MPI(dm_real, term + SV4IDX(1));
                        el_imag[1] = LOCAL_G_HIP_MPI(dm_imag, term + SV4IDX(1));
                        el_real[2] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV4IDX(2));
                        el_imag[2] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV4IDX(2));
                    }
                    else // qubit1 is the remote qubit
                    {
                        el_real[1] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV4IDX(1));
                        el_imag[1] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV4IDX(1));
...
#pragma unroll
                    ...
                    LOCAL_P_HIP_MPI(dm_real, term + SV4IDX(0), res_real[0]);
                    LOCAL_P_HIP_MPI(dm_imag, term + SV4IDX(0), res_imag[0]);
                    LOCAL_P_HIP_MPI(dm_real_remote, term + SV4IDX(3), res_real[3]);
                    LOCAL_P_HIP_MPI(dm_imag_remote, term + SV4IDX(3), res_imag[3]);

                    if (qubit0 == q) // qubit0 is the remote qubit
                    {
                        LOCAL_P_HIP_MPI(dm_real, term + SV4IDX(1), res_real[1]);
                        LOCAL_P_HIP_MPI(dm_imag, term + SV4IDX(1), res_imag[1]);
                        LOCAL_P_HIP_MPI(dm_real_remote, term + SV4IDX(2), res_real[2]);
                        LOCAL_P_HIP_MPI(dm_imag_remote, term + SV4IDX(2), res_imag[2]);
                    }
                    else // qubit1 is the remote qubit
                    {
                        LOCAL_P_HIP_MPI(dm_real_remote, term + SV4IDX(1), res_real[1]);
                        LOCAL_P_HIP_MPI(dm_imag_remote, term + SV4IDX(1), res_imag[1]);
                        LOCAL_P_HIP_MPI(dm_real, term + SV4IDX(2), res_real[2]);
                        LOCAL_P_HIP_MPI(dm_imag, term + SV4IDX(2), res_imag[2]);
                    }
                }
                grid.sync();
                if (tid == 0)
                ...
                if (tid == 0)
                    roc_shmem_ctx_double_put(*p_ctx, dm_imag, dm_imag_remote, per_pe_num, pair_gpu); //TODO: remove dm_real_remote
                                // BARR_NVSHMEM;
            }
        }
            ...
            const IdxType qubit0, const IdxType qubit1)
        {
            ...
            if (tid == 0)
                roc_shmem_ctx_double_get(*p_ctx, dm_real_remote, dm_real, per_pe_num, pair_gpu);
            if (tid == 0)
                roc_shmem_ctx_double_get(*p_ctx, dm_imag_remote, dm_imag, per_pe_num, pair_gpu);
            grid.sync();
                                             ...
                                             const IdxType qubit2, const IdxType qubit3)
        {
            ...
            else // s qubit is non-local
            {
                     ...
                     i += blockDim.x * gridDim.x)
                {
                    ...
                    {
                        el_real[0] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(0));
                        el_real[1] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(1));
                        el_real[2] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(2));
                        el_real[3] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(3));
                        el_real[4] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(4));
                        el_real[5] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(5));
                        el_real[6] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(6));
                        el_real[7] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(7));
                        el_real[8] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(8));
                        el_real[9] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(9));
                        el_real[10] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(10));
                        el_real[11] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(11));
                        el_real[12] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(12));
                        el_real[13] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(13));
                        el_real[14] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(14));
                        el_real[15] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(15));

                        el_imag[0] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(0));
                        el_imag[1] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(1));
                        el_imag[2] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(2));
                        el_imag[3] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(3));
                        el_imag[4] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(4));
                        el_imag[5] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(5));
                        el_imag[6] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(6));
                        el_imag[7] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(7));
                        el_imag[8] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(8));
                        el_imag[9] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(9));
                        el_imag[10] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(10));
                        el_imag[11] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(11));
                        el_imag[12] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(12));
                        el_imag[13] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(13));
                        el_imag[14] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(14));
                        el_imag[15] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(15));
                    }
                    else // qubit2 is remote (not possible qubit0 or 1 is remote)
                    {
                        // if (( (laneid>>1)&1)!=0) el_real_s[j*16+laneid] = LOCAL_G_HIP_MPI(dm_real_remote,addr);
                        // else el_real_s[j*16+laneid] = LOCAL_G_HIP_MPI(dm_real, addr);
                        // laneid = 2,3,6,7,10,11,14,15;

                        el_real[0] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(0));
                        el_real[1] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(1));
                        el_real[2] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(2));
                        el_real[3] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(3));
                        el_real[4] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(4));
                        el_real[5] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(5));
                        el_real[6] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(6));
                        el_real[7] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(7));
                        el_real[8] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(8));
                        el_real[9] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(9));
                        el_real[10] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(10));
                        el_real[11] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(11));
                        el_real[12] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(12));
                        el_real[13] = LOCAL_G_HIP_MPI(dm_real, term + SV16IDX(13));
                        el_real[14] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(14));
                        el_real[15] = LOCAL_G_HIP_MPI(dm_real_remote, term + SV16IDX(15));

                        el_imag[0] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(0));
                        el_imag[1] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(1));
                        el_imag[2] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(2));
                        el_imag[3] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(3));
                        el_imag[4] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(4));
                        el_imag[5] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(5));
                        el_imag[6] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(6));
                        el_imag[7] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(7));
                        el_imag[8] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(8));
                        el_imag[9] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(9));
                        el_imag[10] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(10));
                        el_imag[11] = LOCAL_G_HIP_MPI(dm_imag_remote, term + SV16IDX(11));
                        el_imag[12] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(12));
                        el_imag[13] = LOCAL_G_HIP_MPI(dm_imag, term + SV16IDX(13));
...
#pragma unroll
                        ...
                        if (qubit3 == s) // qubit3 is remote qubit
                        {
                            if ((j & 1) != 0)
                            {
                                LOCAL_P_HIP_MPI(dm_real_remote, term + SV16IDX(j), res_real);
                                LOCAL_P_HIP_MPI(dm_imag_remote, term + SV16IDX(j), res_imag);
                            }
                            else
                            {
                                LOCAL_P_HIP_MPI(dm_real, term + SV16IDX(j), res_real);
                                LOCAL_P_HIP_MPI(dm_imag, term + SV16IDX(j), res_imag);
                            }
                        }
                        else // qubit2 is remote (not possible qubit0 or 1 is remote)
                        {
...
//             ValType *el_real_s = &els[wid * 8 * 16 * 2];          // per warp 8*16 for real TODO: change this 
//             ValType *el_imag_s = &els[wid * 8 * 16 * 2 + 8 * 16]; // per warp 8*16 for imag TODO: change this 
...
//             rocwmma::fragment<rocwmma::matrix_a, 8, 8, 4, ValType, rocwmma::row_major> a_frag_real_up;
//             rocwmma::fragment<rocwmma::matrix_a, 8, 8, 4, ValType, rocwmma::row_major> a_frag_imag_up;
//             rocwmma::fragment<rocwmma::matrix_a, 8, 8, 4, ValType, rocwmma::row_major> a_frag_real_dn;
//             rocwmma::fragment<rocwmma::matrix_a, 8, 8, 4, ValType, rocwmma::row_major> a_frag_imag_dn;
...
//             rocwmma::fragment<rocwmma::accumulator, 8, 8, 4, ValType> c_frag_real_up;
//             rocwmma::fragment<rocwmma::accumulator, 8, 8, 4, ValType> c_frag_imag_up;
//             ;
//             rocwmma::fragment<rocwmma::accumulator, 8, 8, 4, ValType> c_frag_real_dn;
//             rocwmma::fragment<rocwmma::accumulator, 8, 8, 4, ValType> c_frag_imag_dn;
//             ;
...
//         // This is with tensorcore and roc_shmem merged communication optimization
...
//                 if (tid == 0)
//                     roc_shmem_double_get(dm_real_remote, dm_real, per_pe_num, pair_gpu);
//                 if (tid == 0)
//                     roc_shmem_double_get(dm_imag_remote, dm_imag, per_pe_num, pair_gpu);
//                 grid.sync();
...
//                                 {
//                                     if ((laneid & 1) != 0)
//                                         el_real_s[j * 16 + laneid] = LOCAL_G_HIP_MPI(dm_real_remote, addr);
//                                     else
//                                         el_real_s[j * 16 + laneid] = LOCAL_G_HIP_MPI(dm_real, addr);
//                                 }
//                                 else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                                 {
//                                     if (((laneid >> 1) & 1) != 0)
...
//                                 {
//                                     if ((laneid & 1) != 0)
//                                         el_imag_s[j * 16 + laneid] = LOCAL_G_HIP_MPI(dm_imag_remote, addr);
//                                     else
//                                         el_imag_s[j * 16 + laneid] = LOCAL_G_HIP_MPI(dm_imag, addr);
//                                 }
//                                 else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                                 {
//                                     if (((laneid >> 1) & 1) != 0)
...
//                             if ((k0 & 1) != 0)
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real_remote, term0, c_frag_real.x[0]);
//                                 LOCAL_P_HIP_MPI(dm_imag_remote, term0, c_frag_imag.x[0]);
//                             }
//                             else
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real, term0, c_frag_real.x[0]);
//                                 LOCAL_P_HIP_MPI(dm_imag, term0, c_frag_imag.x[0]);
//                             }
//                         }
//                         else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                         {
...
//                             if ((k1 & 1) != 0)
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real_remote, term1, c_frag_real.x[1]);
//                                 LOCAL_P_HIP_MPI(dm_imag_remote, term1, c_frag_imag.x[1]);
//                             }
//                             else
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real, term1, c_frag_real.x[1]);
//                                 LOCAL_P_HIP_MPI(dm_imag, term1, c_frag_imag.x[1]);
//                             }
//                         }
//                         else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                         {
...
//                             if ((k2 & 1) != 0)
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real_remote, term2, c_frag_real.x[0]);
//                                 LOCAL_P_HIP_MPI(dm_imag_remote, term2, c_frag_imag.x[0]);
//                             }
//                             else
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real, term2, c_frag_real.x[0]);
//                                 LOCAL_P_HIP_MPI(dm_imag, term2, c_frag_imag.x[0]);
//                             }
//                         }
//                         else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                         {
...
//                             if ((k3 & 1) != 0)
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real_remote, term3, c_frag_real.x[1]);
//                                 LOCAL_P_HIP_MPI(dm_imag_remote, term3, c_frag_imag.x[1]);
//                             }
//                             else
//                             {
//                                 LOCAL_P_HIP_MPI(dm_real, term3, c_frag_real.x[1]);
//                                 LOCAL_P_HIP_MPI(dm_imag, term3, c_frag_imag.x[1]);
//                             }
//                         }
//                         else // qubit2 is remote (not possible qubit0 or 1 is remote)
//                         {
...
// #endif
                                          ...
                                          const IdxType qubit, const IdxType cur_index)
        {
            ...
            if (i_proc == 0)
            {
                ...
                for (IdxType k = ((IdxType)1 << (n_qubits - 1)); k > 0; k >>= 1)
                {
                    for (IdxType i = tid; i < k; i += blockDim.x * gridDim.x){
                        ValType tmp = m_real[i] + m_real[i + k];
            ...
            grid.sync();
            ValType prob_of_one = LOCAL_G_HIP_MPI(m_real, 0);
            grid.sync();
            ...
            if (tid == 0){
                ...
                roc_shmem_ctx_longlong_p(*p_ctx, &results_gpu[cur_index], res, i_proc);
            }
        }
        //*/
        ...
        __device__ __inline__ void Normalization(ValType *dm_real, ValType *dm_imag)
        {
            ...
            if (i_proc == 0)
            {
                ...
                for (IdxType k = ((IdxType)1 << (n_qubits - 1)); k > 0; k >>= 1)
                {
                    for (IdxType i = tid; i < k; i += blockDim.x * gridDim.x) 
                    {
                        ValType tmp3 = m_real[i] + m_real[i + k];
                        LOCAL_P_HIP_MPI(m_real, i, tmp3);
                    }
                    grid.sync();
                }
            }
            ...
            if (tid == 0 && i_proc != 0) 
            {
                ValType tmp4 = PGAS_G(m_real, 0);
                LOCAL_P_HIP_MPI(m_real, 0, tmp4);
            }
            grid.sync();
            ValType purity = m_real[0];
            for (IdxType i = tid; i < per_pe_work_dm; i += blockDim.x * gridDim.x)
            {
                LOCAL_P_HIP_MPI(dm_real, i, dm_real[i]/purity);
                LOCAL_P_HIP_MPI(dm_imag, i, dm_imag[i]/purity);
            }
        }
        ...
        __device__ __inline__ void MA_GATE(const IdxType repetition, const IdxType cur_index)
        {
            ...
            if (i_proc == 0)
            {
                ...
                {
                    ValType tmp5 = abs(PGAS_G(dm_real, (i << (n_qubits)) + i));
                    LOCAL_P_HIP_MPI(m_real, i, tmp5);
                }
                grid.sync();
                ...
                for (IdxType d = 0; d < (n_qubits); d++)
                {
                    ...
                    for (IdxType k = tid * step; k < n_size; k += step * blockDim.x * gridDim.x) {
                        ValType tmp6 = m_real[k + (1 << d) - 1] + m_real[k + (1 << (d + 1)) - 1];
                        LOCAL_P_HIP_MPI(m_real, (k + (1 << (d + 1)) - 1), tmp6);
                    }
                    grid.sync();
                }
                if (tid == 0)
                ...
                for (IdxType d = (n_qubits)-1; d >= 0; d--)
                {
                    ...
                    {
                        ValType tmp7 = m_real[k + ((IdxType)1 << (d + 1)) - 1];
                        ValType tmp8 =  m_real[k + ((IdxType)1 << d) - 1] + m_real[k + ((IdxType)1 << (d + 1)) - 1];
                        LOCAL_P_HIP_MPI(m_real, (k + ((IdxType)1 << d) - 1), tmp7);
                        LOCAL_P_HIP_MPI(m_real, (k + ((IdxType)1 << (d + 1)) - 1), tmp8);
                    }
                    grid.sync();
                }
                grid.sync();
            ...
            BARR_ROC_SHMEM;
            if (i_proc != 0 && tid == 0)
                roc_shmem_ctx_longlong_get(*p_ctx, results_gpu, results_gpu, repetition, 0);
            BARR_ROC_SHMEM;
        }
        ...
        __device__ __inline__ void RESET_GATE(const IdxType qubit)
        {
            ...
            if (i_proc == 0)
            {
                ...
                for (IdxType k = ((IdxType)1 << (n_qubits - 1)); k > 0; k >>= 1)
                {
                    for (IdxType i = tid; i < k; i += blockDim.x * gridDim.x)
                    {
                        ValType tmp9 = m_real[i] + m_real[i+k];
                        LOCAL_P_HIP_MPI(m_real, i, tmp9);
                    }
            ...
            if (tid == 0 && i_proc != 0)
            {
                ValType tmp10 = PGAS_G(m_real, 0);
                LOCAL_P_HIP_MPI(m_real, 0, tmp10);
            }
            grid.sync();
            ...
            if (prob_of_one < 1.0) // still possible to normalize
            {
                ...
                for (IdxType i = tid; i < per_pe_work_dm; i += blockDim.x * gridDim.x)
                {
                    if ((i & mask) == 0)
                    {
                        LOCAL_P_HIP_MPI(dm_real, i, dm_real[i]*factor);
                        LOCAL_P_HIP_MPI(dm_imag, i, dm_imag[i]*factor);
                    }
                    else
                    {
                        LOCAL_P_HIP_MPI(dm_real, i, 0);
                        LOCAL_P_HIP_MPI(dm_imag, i, 0);
                    }
                }
            }
            else
            {
                for (IdxType i = tid; i < per_pe_work_dm; i += blockDim.x * gridDim.x)
                {
                    if ((i & mask) == 0)
                    {
                        ...
                        LOCAL_P_HIP_MPI(dm_real, i, dm_real[dual_i]);
                        LOCAL_P_HIP_MPI(dm_imag, i, dm_imag[dual_i]);
                        LOCAL_P_HIP_MPI(dm_real, dual_i, 0);
                        LOCAL_P_HIP_MPI(dm_imag, dual_i, 0);
                    }
                }
            }
            BARR_ROC_SHMEM;
