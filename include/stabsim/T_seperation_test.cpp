...
void appendQASMToCircuit(std::shared_ptr<NWQSim::Circuit>& circuit, const std::string& filename) 
{
    ...
    while (std::getline(file, line)) 
    {
        ...
        if (line.empty() || line.find("OPENQASM") != std::string::npos || line.find("qreg") != std::string::npos || line.find("creg") != std::string::npos || line.find("#include") != std::string::npos)
            continue;
...
int main(){
    ...
    appendQASMToCircuit(circuit, "/Users/garn195/Project Repositories/PauliTrans/test_circuits/qft_n18.qasm");
