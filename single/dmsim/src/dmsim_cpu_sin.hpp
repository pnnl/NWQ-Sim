...
// BSD Lincese.
// ---------------------------------------------------------------------------
// File: dmsim_cpu_sin.hpp
// Single thread CPU based density-matrix simulation 
// ---------------------------------------------------------------------------
#ifndef DMSIM_CPU_SIN_HPP_
#define DMSIM_CPU_SIN_HPP_
#include <assert.h>
#include <vector>
#include <math.h>
#include <cstdlib>
...
{
public:
    Simulation(IdxType _n_qubits=0, IdxType _n_cpus=1) 
        : n_gates(0), 
        sv_real(NULL),
        sv_imag(NULL),
        m_real(NULL),
    ...
    void sim()
    {
        ...
        printf("============== DMSim_cpu_sin ===============\n");
#ifdef USE_AVX512
        printf("nqubits:%llu, ngates:%llu, ncores(avx512):1, sim:%.3lf ms\n", n_qubits, n_gates, sim_time);
#else
        printf("nqubits:%llu, ngates:%llu, ncores:1, sim:%.3lf ms\n", n_qubits, n_gates, sim_time);
#endif
        printf("=====================================\n");
#endif
        reset_circuit();
...
inline void M_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, const ValType rand, const IdxType qubit)
{
    ...
    {
        if ((i&mask)!=0)
            prob_of_one += abs(sv_real[(i<<(sim->n_qubits))+i]); //diagonal element
    }
    if (rand < prob_of_one)
    {
...
inline void MA_GATE(const Simulation* sim, ValType* sv_real, ValType* sv_imag, const IdxType repetition)
{
    ...
    for (IdxType i=1; i<(((IdxType)1<<(sim->n_qubits))+1); i++)
        m_real[i] = m_real[i-1] + abs(sv_real[((i-1)<<(sim->n_qubits))+(i-1)]); //diagonal element
