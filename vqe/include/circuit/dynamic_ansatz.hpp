    ...
    class DynamicAnsatz: public Ansatz {
      ...
      public:
        ...
        void make_op_pool(Transformer tf, IdxType seed, IdxType pool_size) {
          if ((pool_type == PoolType::Fermionic) || (pool_type == PoolType::Singlet_GSD) || (pool_type == PoolType::Fermionic_Origin)) {
            if (pool_type == PoolType::Fermionic) {
              generate_fermionic_excitations(fermi_op_pool, env);
              ansatz_name = "UCCSD Minimal (Dynamic)";
            } else if (pool_type == PoolType::Singlet_GSD) {
              generate_singlet_gsd_excitations(fermi_op_pool, env);
              ansatz_name = "Singlet GSD (Dynamic)";
            } else if (pool_type == PoolType::Fermionic_Origin) {
        ...
        virtual std::string get_operator_string(IdxType operator_index) {
          std::string opstring = "";
          if (pool_type == PoolType::Pauli || pool_type == PoolType::MinimalPauli) {  
          ...
          } else { // MZ: Pauli pool is the minority now
            size_t index = 0;
