...
namespace NWQSim
{
  namespace VQE
  {
    ...
    public:
      UCCSDmin(const MolecularEnvironment &_env, Transformer _qubit_transform, IdxType _trotter_n = 1, IdxType _symm_level = 3) : env(_env),
        ...
        for (IdxType i = 0; i < env.n_occ; i++)
        {
          ...
          for (IdxType r = 0; r < env.n_virt; r++)
          {
            ...
            for (IdxType j = 0; j < env.n_occ; j++)
            {
              ...
              {
                if (!((symm_level < 2) || (i == j && r == s) || (i == j && r < s) || (i < j)))
                  continue;
                FermionOperator s_virt_cre_dw(s, Virtual, Down, Creation, env.xacc_scheme);
