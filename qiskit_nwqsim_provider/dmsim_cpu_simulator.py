...
"""Backend for the DM-Sim CPU simulation environment."""
...
class DMSimWrapper:
    """Converter to and from DMSim_cpu simulator"""

    def __init__(self, qobj, shell):
        self.shots = qobj.config.shots
        self.seed = qobj.config.seed_simulator
        self.n_qubits = qobj.config.n_qubits
        self.qobj = qobj
        self.memory_mapping = self._build_memory_mapping()
        self.sim = dmsim.Simulation(self.n_qubits, shell.n_cpus)
        self.sim.set_seed(self.seed)
        if (shell.do_noise_1q):
            self.sim.set_noise_1q_sop(shell.noise_1q)
        if (shell.do_noise_2q):
            self.sim.set_noise_2q_sop(shell.noise_2q)
        self.additional_output_data = []

    def final_statevector(self):
        return self.sim.get_dm()
...
#======================================== DMSim Simulator ================================
class DMSimCpuSimulator(BaseBackend):
    """Python interface to NWQSim's DMSim cpu simulator"""
    ...
    DEFAULT_CONFIGURATION = {
        ...
        'basis_gates': ['x', 'y', 'z', 'h', 's', 'sdg', 't', 'tdg', 'ri', 'rx', 'ry', 'rz', 'sx', 'p', 'u', 'cx', 'cy', 'cz', 'ch', 'cs', 'csdg', 'ct', 'ctdg', 'cri', 'crx', 'cry', 'crz', 'csx', 'cp', 'cu', 'id', 'swap', 'm', 'ma', 'reset'],
        'gates': [
            {
                'name': 'TODO',
                'parameters': [],
