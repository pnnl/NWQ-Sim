...
#include "../../single/svsim/src/svsim_cpu_omp.hpp"
...
namespace NWQSim
{
    class NWQBackendSVCpuOmp : public NWQBackend
    {
    public:
        ...
        virtual double calcExpectationValueZ(const std::vector<size_t> &in_bits) override
        {
            ...
            for (unsigned long long i = 0; i < m_sim->dim; ++i)
            {
                result += (hasEvenParity(i, in_bits) ? 1.0 : -1.0) *
                          (m_sim->sv_real[i] * m_sim->sv_real[i] + m_sim->sv_imag[i] * m_sim->sv_imag[i]);
            }
    ...
    {
        return std::make_shared<NWQBackendSVCpuOmp>();
    }
